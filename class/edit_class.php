...
$cls_obj     =new classes();
if(isset($_GET['cls_id'])>0){
  $old_class_id=$_GET['cls_id'];
  $res_data=$cls_obj->get_class_by_id($old_class_id);
  $i=0;
  while ($class=mysqli_fetch_assoc($res_data)) {
    echo "<pre>";
      print_r($class);
      list($sem,$sem_no)=preg_split("/[()]+/",$class['class_name']);
      $deg_id=$class['degree_id'];
      $course_id[$i]=$class['course_id'];
    echo "</pre>";
    ++$i;
  }
        ...
        <form method="post" action="">
          <table class="table table-bordered">
            ...
            <tr>
              <th>Class Name:</th>
              <td>
                <input type="text" name="cls_name" value="<?= $sem ?>" class="form-control" required autofocus>
              </td>
            </tr>
            <tr>
              <th>Semester Number:</th>
              <td><input type="text" class="form-control" value="<?= $sem_no ?>" name="cls_sem" required></td>
            </tr>
            <tr>
              ...
              <td>
                <select class="form-control" name="degree">
                  <?php while ($degree=mysqli_fetch_assoc($degree_data)) { ?>
                    <option value="<?= $degree['degree_id'] ?>" <?php if ($deg_id==$degree['degree_id']): ?>
                      selected
                    <?php endif; ?>><?= $degree['degree_name']?>  (<?= $degree['degree_subject_name']?>)</option>
                  <?php } ?>
                </select>
              </td>
            </tr>
                  ...
                  <?php while ($course=mysqli_fetch_assoc($courses_data)) { ?>
                    <li>
                      <label for="course[<?= $course['course_id'] ?>]">
                        <input type="checkbox" name="course[<?= $course['course_id'] ?>]" <?php foreach ($course_id as $key => $value):
                          if ($value==$course['course_id']) {
                          ?>
                            checked
                          <?php
                              }
                              endforeach; ?> value="<?= $course['course_id'] ?>">
